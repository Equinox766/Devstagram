...
@section('content')
    <div class="container md:flex mx-auto">
                    ...
                    @if($post->user_id === auth()->user()->id )
                        <form action="{{ route('posts.destroy', $post) }}" method="POST">
                            ...
                            <input
                                ...
                                class="bg-red-500 hover:bg-red-600 p-2 rounded cursor-pointer text-white font-bold mt-4"
                            >
                        </form>
                    @endif
                @endauth
        ...
        <div class="md:w-1/2 p-5">
            <div class="shadow bg-white mb-5 p-4">
                @auth()
                    <p class="text-xl font-bold text-center mb-4">Agrega un nuevo comentario</p>
                    @if(session('mensaje'))
                        <p class="bg-green-500 text-white p-2 rounded-lg uppercase text-center font-bold">
                            {{ session('mensaje')  }}
                        </p>
                    @endif
